
	<div class="content__head" id="sec-content-head">
		<div class="row">
			<div class="col-xs-12">
				<div class="arabic">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</div>
				<div class="content__head__title title_style">Bismillahirrahmanirrahim</div>
				<p class="content__head__body">
					Assalamu'alaikum Warahmatullahi Wabarakaatuh<br>
					Maha Suci Allah yang telah menciptakan makhluk-Nya<br>
					berpasang-pasang<br>
					Ya Allah, perkenankanlah putra-putri kami:
				</p>
			</div>
		</div>
	</div>

	<center>
	<div class="content__body">

		<div class="row" style="align-items: center">

			<div class="col-xs-12 col-md-5">
				<div class="content__body__person" id="sec-content-body-p1">
					<center>
					<div class="person">
						<img src="<?php echo e($imagesPath . 'p1.webp'); ?>" class="person__image" alt="" title="">
					</div>
					</center>
					<div class="person_name">
						<span class="person_name_style"><?php echo e($brideFullName1); ?></span>
						<div class="person_ig">
							<a href="<?php echo e($ig1); ?>" target="_blank"><span><i class="bi bi-instagram"></i> <?php echo e($brideIg1); ?></span></a>
						</div>
					</div>
					<div class="person_parent">
						<span>Putri dari,</span><br>
						<span><?php echo e($parent1); ?></span>
					</div>
				</div>
			</div>

			<div class="col-xs-12 col-md-2">
				<div class="content__body__separator">
					<span class="content__body__separator__style">&</span>
				</div>
			</div>

			<div class="col-xs-12 col-md-5">
				<div class="content__body__person" id="sec-content-body-p2">
					<center>
					<div class="person">
						<img src="<?php echo e($imagesPath . 'p2.webp'); ?>" class="person__image">
					</div>
					</center>
					<div class="person_name">
						<span class="person_name_style"><?php echo e($brideFullName2); ?></span>
						<div class="person_ig">
							<a href="<?php echo e($ig2); ?>" target="_blank"><span><i class="bi bi-instagram"></i> <?php echo e($brideIg2); ?></span></a>
						</div>
					</div>
					<div class="person_parent">
						<span>Putra dari,</span><br>
						<span><?php echo e($parent2); ?></span>
					</div>
				</div>
			</div>
			
		</div>

	</div>
	</center>
<?php /**PATH E:\xampp\htdocs\wedding\application\modules/Home/views/partial_detail/content.blade.php ENDPATH**/ ?>